...
public class CameraRaycastController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        RaycastHit myRayHit;
        if (Physics.Raycast(myRay, out myRayHit, 10000.0f))
        {
            //i shot out a ray and it hit something
            //Debug.Log("I hit something");
            GameObject hitObject = myRayHit.transform.gameObject;
            InteractionController interactionController = hitObject.GetComponent<InteractionController>();
            if (interactionController && interactionController.enabled)
            {
